# Historique de développement — PertFlow

> Archive du récit session par session, extraite de `CLAUDE.md` (allègement du
> 23/07/2026). Le contrat durable (règles, modèle de données, moteur PERT, pièges
> LiteGraph, rituel de fin de session) reste dans `CLAUDE.md` ; ce fichier ne
> contient que l'historique détaillé, consultable à la demande.
>
> **Règle** : à la clôture d'une session, ajouter le détail ICI et n'inscrire qu'UNE
> ligne au tableau de statut de `CLAUDE.md` (§ « État d'avancement »).
